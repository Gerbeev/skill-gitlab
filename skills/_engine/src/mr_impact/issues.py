...
def template_slots(template):
    ...
    for i, line in enumerate(lines, 1):
        ...
        if re.fullmatch(r"\s*\*\*.+\*\*\s*", line):
            ...
            for ordinal, match in enumerate(matches, 1):
                identifier = f"L{i}" if len(matches) == 1 else f"L{i}P{ordinal}"
                slots.append(Slot(identifier, i, match[1], heading, "placeholder", match.start(), match.end()))
    ...
    for line_no, line in enumerate(lines, 1):
        ...
        for slot in by_line.get(line_no, []):
            slot.ancestors = tuple(title for _, title in headings)
    return sorted(slots, key=lambda slot: (slot.line, slot.start_column))
...
def template_instructions(template):
    return [{"id": f"I{number}", "start_line": template.count("\n", 0, match.start()) + 1,
             "end_line": template.count("\n", 0, match.end()) + 1, "text": match[1].strip()}
            for number, match in enumerate(re.finditer(r"<!--([\s\S]*?)-->", template), 1)]


def optional_empty(slot, instructions, template):
    """Recognize a narrow empty-field directive; other prose requires agent review."""
    lines = template.splitlines()
    section_start = max((i for i in range(1, slot.line + 1) if re.match(r"^#{1,6}\s", lines[i - 1])), default=1)
    section_end = next((i for i in range(slot.line + 1, len(lines) + 1) if re.match(r"^#{1,6}\s", lines[i - 1])), len(lines) + 1)
    return any(section_start <= instruction["start_line"] < section_end and
               re.search(r"leave (?:it |this (?:field|section) )?(?:empty|blank) (?:when|if) (?:no (?:supporting )?evidence exists|unsupported|no evidence)",
                         instruction["text"], re.I) for instruction in instructions)


def protected_slot(slot):
    categories = [category(slot.label)] + [category(title) for title in reversed(slot.ancestors or (slot.section,))]
    return any(value in {"requirement", "scope"} for value in categories)
...
def validate_interpretation(plan, template, slots, sources):
    ...
    if hashes is not None and hashes != source_fingerprints(sources):
        raise EngineError("Interpretation sources changed; inspect current sources again")
    decisions = plan.get("source_decisions", {})
    if not isinstance(decisions, dict) or set(decisions) - set(sources):
        raise EngineError("Invalid source decisions")
    for decision in decisions.values():
        if not isinstance(decision, dict) or decision.get("use") not in {"included", "excluded"} or not decision.get("reason"):
            raise EngineError("Source decisions require included/excluded and a reason")
    if plan.get("reviewed"):
        if hashes != source_fingerprints(sources) or set(decisions) != set(sources):
            raise EngineError("Completed review requires current source fingerprints and decisions for every source")
        expected = {item["id"] for item in template_instructions(template)}
        if set(plan.get("reviewed_instructions", [])) != expected:
    ...
    for slot, items in fills.items():
        ...
        for item in items:
            ...
            for citation in citations:
                ...
                if decisions.get(citation["file"], {}).get("use") == "excluded":
                    raise EngineError("Interpretation cites an excluded source")
...
def analyze_issue(root: Path, output: Path, scope: Path | None = None, template: Path | None = None,
                  interpretation: Path | None = None, require_review=False):
    ...
    plan = read_json(interpretation) if interpretation else None
    slots = template_slots(template_text)
    fills, empty_slots = validate_interpretation(plan, template_text, slots, sources) if plan is not None else (None, set())
    decisions = plan.get("source_decisions", {}) if plan else {}
    included = {name: text for name, text in sources.items() if decisions.get(name, {}).get("use") != "excluded"}
    facts = extract_facts(included)
    instructions = template_instructions(template_text)
    reviewed = bool(plan and plan.get("reviewed"))
    if require_review and not reviewed:
        raise EngineError("A completed, source-bound semantic interpretation is required")
    generated = template_text.splitlines()
    used, unresolved, records = set(), [], []
    replacements = {}
    def replace_slot(slot, text):
        replacements.setdefault(slot.line, []).append((slot.start_column, slot.end_column, text))
    for slot in slots:
        ...
        if fills is not None:
            ...
            candidates = [f for f in _matching(slot, facts) if slot.prefix == "placeholder" or (f.file, f.line) not in used]
        ...
        if not texts and optional_empty(slot, instructions, template_text):
            if slot.prefix == "placeholder":
                replace_slot(slot, "")
            continue
        if not texts:
        ...
        if slot.prefix == "append":
            ...
            replace_slot(slot, "; ".join(texts))
        else:
            generated[slot.line - 1] = "\n".join(slot.prefix + t for t in texts)
    for line, edits in replacements.items():
        for start, end, text in sorted(edits, reverse=True):
            generated[line - 1] = generated[line - 1][:start] + text + generated[line - 1][end:]
    clean = re.sub(r"<!--[\s\S]*?-->", "", "\n".join(generated))
    clean = re.sub(r"\n{3,}", "\n\n", clean).strip() + "\n"
    ...
    report += [f"Review status: {'reviewed' if reviewed else 'draft; semantic review required'}.", ""]
    grouped = {}
    for fact in facts:
        grouped.setdefault((fact.kind, fact.topic, fact.text), []).append(f"{fact.file}:{fact.line}")
    for (kind, topic, text), citations in list(grouped.items())[:120]:
        report.append(f"- **{kind} / {topic}**: {text} ({'; '.join(citations)}).")
    if len(grouped) > 120:
        report.append(f"- {len(grouped) - 120} additional distinct statements omitted from this summary; inspect the listed sources.")
    if not facts:
    ...
    report += ["", "## Source decisions", ""]
    report += [f"- `{name}`: {decisions.get(name, {}).get('use', 'included provisionally')}; "
               f"{decisions.get(name, {}).get('reason', 'Awaiting semantic relevance review')}" for name in sources]
    if not reviewed:
        report += ["", "## Pending template instruction review", ""]
        report += [f"- {item['id']} at template lines {item['start_line']}-{item['end_line']}: agent review required."
    ...
    return {"outputs": [str(output / "00-issue-analysis.md"), str(output / "01-generated-issue.md")],
            "status": "reviewed" if reviewed else "draft", "source_sha256": source_fingerprints(sources),
...
def update_issue(issue: Path, analysis: Path, output: Path, target="Unspecified local Issue", validation: Path | None = None):
    ...
    summary = [f"Changed files: {len(context.get('changes', []))}. Revisions: `{context.get('base') or ('empty tree' if context.get('base_kind') == 'empty_tree' else 'external patch')}..{context.get('head')}`.", ""]
    ...
    lines += [f"- {item['id']} ({item['status']}): {item['reason']} {item['action']}" for item in context.get("follow_up", [])]
