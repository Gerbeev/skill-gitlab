            ...
            for item in ast.iter_child_nodes(node):
                if isinstance(item, (ast.ClassDef, ast.FunctionDef, ast.AsyncFunctionDef)):
                    ...
                    key = c.symbol(name, "class" if isinstance(item, ast.ClassDef) else "function",
                                   ...
                                   (ast.dump(item.args, include_attributes=False) + " -> "
                                    + (ast.dump(item.returns, include_attributes=False) if item.returns else "None")
                                    + (" async" if isinstance(item, ast.AsyncFunctionDef) else ""))
                                   if isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef))
                                   else ";".join(ast.dump(b, include_attributes=False) for b in item.bases), C.EXACT)
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, (ast.Import, ast.ImportFrom)):
                ...
                if isinstance(node, ast.ImportFrom) and node.level:
                    prefix = list(PurePosixPath(c.path).parent.parts)
                    if node.level > len(prefix):
                        c.result.warnings.append(f"Relative import escapes package: {c.path}:{node.lineno}")
                        continue
                    prefix = prefix[:len(prefix) - node.level + 1]
                    if node.module is None:
                        modules = [a.name for a in node.names if a.name != "*"]
                for module in filter(None, modules):
                    target = "file://" + "/".join(prefix + module.split(".")) + ".py"
    ...
    def extract(self, c):
        ...
        for name, start, fields in jobs:
            ...
            if "profile" in fields:
                value, line = fields["profile"]
                c.reference(value, line, key, kind="CONFIGURES", confidence=C.PROBABLE)
...
class RuntimeAdapter:
    """Static entrypoint discovery for cron and systemd, plus explicit event calls."""
    name = "runtime-entrypoints"

    def accepts(self, path):
        return PurePosixPath(path).suffix.lower() in {".cron", ".service", ".timer", ".py", ".cs", ".scala"} or PurePosixPath(path).name == "crontab"

    def extract(self, c):
        suffix = PurePosixPath(c.path).suffix.lower()
        if suffix in {".service", ".timer"}:
            c.node(c.file_key, "SERVICE" if suffix == ".service" else "PROCESS", c.path, 1, C.STRONG, True)
            for line, text in enumerate(c.lines, 1):
                if text.startswith("ExecStart="):
                    self.command(c, c.file_key, text.split("=", 1)[1], line)
                elif suffix == ".timer" and text.startswith("Unit="):
                    unit = text.split("=", 1)[1].strip()
                    c.reference("./" + unit, line, c.file_key, kind="TRIGGERS", confidence=C.STRONG)
            if suffix == ".timer" and not any(line.startswith("Unit=") for line in c.lines):
                c.reference("./" + PurePosixPath(c.path).with_suffix(".service").name, 1, c.file_key, kind="TRIGGERS", confidence=C.PROBABLE)
        elif suffix == ".cron" or PurePosixPath(c.path).name == "crontab":
            for line, text in enumerate(c.lines, 1):
                text = text.strip()
                if not text or text.startswith("#"):
                    continue
                parts = text.split()
                offset = 1 if text.startswith("@") else 5
                if len(parts) <= offset or not (text.startswith("@") or re.fullmatch(r"[0-9*/?,\-]+", parts[0])):
                    continue
                key = f"cron://{c.path}:{line}"
                c.node(key, "WORKFLOW", f"{c.path}:{line}", line, C.PROBABLE, True)
                c.edge(c.file_key, key, "DEFINES", line, self.name, C.PROBABLE)
                self.command(c, key, " ".join(parts[offset:]), line)
        else:
            for line, text in enumerate(c.lines, 1):
                if text.lstrip().startswith(("#", "//")):
                    continue
                for match in re.finditer(r"\b(publish_event|consume_event)\(\s*[\"']([\w./-]+)[\"']", text):
                    key = canonical("event", match[2])
                    c.node(key, "EVENT", match[2], line, C.CANDIDATE, True, {"reference_only": True})
                    role = "PUBLISHES_EVENT" if match[1] == "publish_event" else "CONSUMES_EVENT"
                    c.edge(c.owner(line), key, role, line, self.name, C.CANDIDATE)

    def command(self, c, owner, command, line):
        matched = False
        for token in re.findall(r"[\w./$%-]+\.(?:py|ps1|sh|bat|cmd|jar|dll|exe)\b", command):
            c.reference(token, line, owner, confidence=C.PROBABLE)
            matched = True
        if not matched:
            c.result.warnings.append(f"Unresolved runtime command at {c.path}:{line}")


ADAPTERS: tuple[Adapter, ...] = (PythonAdapter(), StructureAdapter(), SqlAdapter(), AutosysAdapter(), ConfigAdapter(), RuntimeAdapter(), GenericAdapter())
