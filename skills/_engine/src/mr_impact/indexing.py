    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as stream:
            ...
            for row in rows:
                if not first:
                    stream.write(",\n")
                stream.write(json.dumps(redact_value(dict(row)), ensure_ascii=True))
                first = False
            stream.write("\n]\n")
...
def build_index(root: Path, cache: Path | None = None, mode="deep", ref="HEAD", worktree=False,
    ...
    with Store(directory / "repository-index.sqlite") as store:
        ...
        with store.db:
            ...
            for row in store.db.execute("SELECT id,target,evidence FROM edges WHERE type='IMPORTS'").fetchall():
                original = json.loads(row["evidence"]).get("import_target", row["target"])
                target = original
                if original.startswith("file://") and original.endswith(".py"):
                    module = original[len("file://"):]
                    package = module[:-3] + "/__init__.py"
                    if module not in selected and package in selected:
                        target = "file://" + package
                if target != row["target"]:
                    store.db.execute("UPDATE edges SET target=? WHERE id=?", (target, row["id"]))
            store.db.execute("""INSERT OR IGNORE INTO nodes
                SELECT target,'UNRESOLVED',target,0,'{}' FROM edges WHERE type='IMPORTS'""")
            store.cleanup()
