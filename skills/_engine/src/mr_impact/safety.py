...
def read_text(path: Path, limit: int = 2_000_000) -> str:
    ...
    with path.open("rb") as stream:
        ...
        if len(data) != size:
            raise EngineError("Input size changed during read")
    if b"\x00" in data:
...
def write_text(path: Path, content: str, *, redact_content=True):
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8", newline="\n") as stream:
            stream.write(redact(content) if redact_content else content)
...
def redact_value(value):
    """Redact values before serialization so JSON syntax and escaping stay valid."""
    if isinstance(value, dict):
        return {key: "[REDACTED]" if re.fullmatch(r"password|token|secret|api[_-]?key", str(key), re.I)
...
def write_json(path: Path, value):
    write_text(path, json.dumps(redact_value(value), indent=2, ensure_ascii=True) + "\n", redact_content=False)
