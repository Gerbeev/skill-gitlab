    ...
    try:
        ...
        for offset in range(0, len(ordered), 400):
            ...
            rows = db.execute(f"""WITH ranked AS (
                SELECT *, ROW_NUMBER() OVER (PARTITION BY entity ORDER BY confidence DESC,role,source) position
                FROM dependencies WHERE repository=? AND entity IN ({placeholders}) AND confidence>=?)
                SELECT * FROM ranked WHERE position=1 ORDER BY entity""", (repository, *chunk, confidence))
            matches.extend(dict(row) | {"evidence": json.loads(row["evidence"])} for row in rows)
        return matches
    finally:
        db.close()
