...
provide a source-bound, evidence-cited interpretation before delivering a completed result.
Use `inspect-issue` to obtain current template slots, instruction IDs, and source hashes.
Record included/excluded source decisions and review every template instruction.
Regenerate with `--interpretation INTERNAL_JSON --require-review`; a standalone
deterministic result is a draft, not a completed semantic review.
